#!/usr/bin/env python3
"""
Add Dashboard Endpoint to Existing API Gateway

This script adds a dashboard endpoint to the existing API Gateway
so judges can access the dashboard through the same API endpoint.
"""
...
def add_dashboard_to_lambda():
    """Add dashboard endpoint to the existing Lambda function."""
    print("🔧 Adding dashboard endpoint to Lambda function...")
    
    # Get the dashboard HTML
    dashboard_html = create_dashboard_html()
    
    # Create a simple Lambda function code that serves the dashboard
    lambda_code = f'''
import json
import base64
...
'''
    ...
    print("✅ Dashboard Lambda code created")
    print("📄 File saved as: dashboard_lambda.py")
    
    return lambda_code


def update_existing_lambda():
    """Update the existing Lambda function to include dashboard."""
    print("🔄 Updating existing Lambda function...")
    
    lambda_client = boto3.client('lambda')
    
    # Try to find the existing Lambda function
    function_names = [
        'incident-commander-api',
        'incident-commander-health-development',
        'incident-commander-detection-agent-development'
    ]
    ...
    for function_name in function_names:
        try:
            ...
            print(f"📍 Function code location: {code_url}")
            
            # For now, just create the dashboard code file
            # In a real deployment, we would update the function code
            add_dashboard_to_lambda()
            
            print(f"ℹ️  To update the function, upload the new code:")
            print(f"   aws lambda update-function-code --function-name {function_name} --zip-file fileb://dashboard_lambda.zip")
            
            return function_name
            
        except Exception as e:
            print(f"⚠️  Function {function_name} not found: {e}")
            continue
    
    print("❌ No existing Lambda functions found")
    return None


def create_simple_api_response():
    """Create a simple API response for testing."""
    print("🔧 Creating simple API response...")
    
    # Create a simple Python file that can be deployed as Lambda
    simple_lambda = '''
import json

def lambda_handler(event, context):
    """Simple Lambda handler for dashboard and API."""
    
    path = event.get('path', '/')
    method = event.get('httpMethod', 'GET')
    
    # Dashboard HTML
    dashboard_html = """''' + create_dashboard_html().replace('"""', '\\"\\"\\"') + '''"""
    ...
    if path == '/health':
        return {
            ...
            'body': json.dumps({
                'status': 'healthy',
                'message': 'Incident Commander Dashboard is operational',
    ...
    return {
        ...
        'body': json.dumps({
            'message': 'Incident Commander API',
            'path': path,
            'dashboard_url': '/dashboard'
        })
    }
'''
    ...
    with open('simple_dashboard_lambda.py', 'w') as f:
        f.write(simple_lambda)
    
    print("✅ Simple Lambda code created: simple_dashboard_lambda.py")
    return simple_lambda


if __name__ == "__main__":
    print("🚀 Adding dashboard to API Gateway...")
    
    # Create the dashboard Lambda code
    create_simple_api_response()
    
    # Try to update existing Lambda
    function_name = update_existing_lambda()
    
    if function_name:
        print(f"\n🎉 Dashboard code prepared for Lambda function: {function_name}")
    else:
        print(f"\n📝 Dashboard code created as standalone files")
    
    print(f"\n📋 Next steps:")
    print(f"1. Package the Lambda code: zip -r dashboard_lambda.zip simple_dashboard_lambda.py")
    print(f"2. Update Lambda function: aws lambda update-function-code --function-name <function-name> --zip-file fileb://dashboard_lambda.zip")
    print(f"3. Test the dashboard: curl https://your-api-gateway-url/dashboard")