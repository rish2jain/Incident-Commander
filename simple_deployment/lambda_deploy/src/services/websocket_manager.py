...
class WebSocketManager:
    ...
    async def broadcast_incident_update(self, incident: Incident, phase: str):
        ...
        """
        logger.info(f"Broadcasting incident update for {incident.id}, phase: {phase}")
        ...
        try:
            ...
            logger.info(f"Got incident status for {incident.id}: {incident_status is not None}")
        except Exception as e:
        ...
        logger.info(f"Sending incident_update message with {len(list(self.incident_flows.values()))} active incidents")
        await self._queue_broadcast(message)
    ...
    async def _execute_demo_scenario(self, demo_controller, scenario_type, connection_id: str):
        ...
        try:
            ...
            scenario_error_message = WebSocketMessage(
                type='demo_scenario_error',
                timestamp=datetime.utcnow(),
                data={'error': str(e)},
                priority=2
            )
            await self.broadcast(scenario_error_message)
